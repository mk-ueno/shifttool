...
function calculateTotalHours() {
    ...
    lines.forEach((line, index) => {
        ...
        if (workMatch && currentDate) {
            ...
            if (startHours >= 22 || endHours < 7) {
                const nightStart = Math.max(startHours, 22);
                const nightEnd = Math.min(endHours, 7) + (endHours < 7 ? 24 : 0);
                totalNightHours += (nightEnd - nightStart) * 60;
            }
            ...
            const commuteCost = parseFloat(document.getElementById('commute-cost').value) || 0;
            totalCommuteCost += commuteCost * 2;

            totalClothingAllowance += 500;
        }
    });
    ...
    const nightWage = hourlyWage * 1.25;
    const nightSalary = (totalNightHours / 60) * nightWage;

    const estimatedSalary = (totalWorkMinutes / 60 * hourlyWage) + totalCommuteCost + totalClothingAllowance + nightSalary;
    document.getElementById('estimated-salary').innerText = Math.max(estimatedSalary, 0).toFixed(0);
}
